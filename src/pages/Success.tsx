...
import { ArrowRight, CheckCircle2, ExternalLink, LoaderCircle, ShieldCheck } from 'lucide-react';
...
type GenerateResponse = { orderId: string; status: string; siteUrl?: string; error?: string };
...
export const Success = () => {
  ...
  const initialCadence = (params.get('cadence') || 'month').trim().toLowerCase() === 'year' ? 'year' : 'month';
  ...
  return (
    <div className="section-shell pt-14">
      ...
      <div className="content-grid gap-10 lg:max-w-5xl">
        <div className="mx-auto w-full max-w-4xl rounded-[32px] border border-white/10 bg-white/[0.05] p-6 shadow-[0_35px_120px_rgba(0,0,0,0.35)] backdrop-blur-2xl sm:p-8">
          <div className="space-y-6">
            <div className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/[0.05] px-4 py-2 text-xs font-semibold uppercase tracking-[0.28em] text-slate-300">
              <ShieldCheck className="h-4 w-4 text-emerald-300" />
              Post-purchase setup
            </div>

            <div className="space-y-3">
              <h1 className="section-title max-w-3xl">Payment received. Now we build your site.</h1>
              <p className="section-copy max-w-2xl">
                This page verifies your checkout, collects your business details, and generates your hosted starter site link.
              </p>
            </div>

            <div className={`rounded-3xl border px-4 py-4 text-sm ${stage === 'error' ? 'border-rose-400/30 bg-rose-400/10 text-rose-100' : stage === 'done' ? 'border-emerald-400/30 bg-emerald-400/10 text-emerald-50' : 'border-white/10 bg-slate-950/60 text-slate-200'}`}>
              <div className="flex items-center gap-3">
                {(stage === 'verifying' || stage === 'creating-order' || stage === 'generating') ? (
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <div className="rounded-2xl border border-white/10 bg-slate-950/60 p-4 text-sm text-slate-300">
                <div className="text-xs font-semibold uppercase tracking-[0.28em] text-slate-400">Session</div>
                <div className="mt-2 break-all font-mono text-xs text-slate-200">{sessionId || 'missing'}</div>
              </div>
              <div className="rounded-2xl border border-white/10 bg-slate-950/60 p-4 text-sm text-slate-300">
                <div className="text-xs font-semibold uppercase tracking-[0.28em] text-slate-400">Plan</div>
                <div className="mt-2 text-base font-semibold capitalize text-white">{plan}</div>
              </div>
            </div>
          </div>
          ...
          <form onSubmit={handleGenerate} className="mt-8 grid gap-5">
            ...
            <div className="flex flex-col gap-3 sm:flex-row">
              <button
                type="submit"
                disabled={stage === 'verifying' || stage === 'creating-order' || stage === 'generating'}
                className="hero-primary-button disabled:cursor-not-allowed disabled:opacity-60"
              >
                {stage === 'creating-order' || stage === 'generating' ? 'Working…' : 'Generate my website'}
                <ArrowRight className="h-4 w-4" />
              </button>
              <Link to="/pricing" className="hero-secondary-button justify-center">
                Back to pricing
              </Link>
            </div>

            <p className="text-xs leading-5 text-slate-400">
              All sales are final and non-refundable. You remain responsible for your site content, claims, and compliance.
            </p>
          </form>
          ...
          {siteUrl ? (
            ...
            >
              <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  <div className="text-xs font-semibold uppercase tracking-[0.28em] text-emerald-100/80">Your site is ready</div>
                  <a href={siteUrl} target="_blank" rel="noreferrer" className="mt-2 inline-flex items-center gap-2 text-base font-semibold text-white underline decoration-emerald-300/40 underline-offset-4">
                    {siteUrl}
                    <ExternalLink className="h-4 w-4" />
                  </a>
                  {orderId ? <div className="mt-2 text-xs text-emerald-50/80">Order ID: {orderId}</div> : null}
                </div>
                <Link to={`/setup?provider=stripe&plan=${encodeURIComponent(plan)}&session_id=${encodeURIComponent(sessionId)}`} className="hero-secondary-button justify-center border-emerald-300/30 text-emerald-50 hover:border-emerald-200/60">
