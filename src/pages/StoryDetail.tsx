...
import { Calendar, Clock, ArrowLeft, Share2, Play } from 'lucide-react';
...
export const StoryDetail = () => {
  ...
  if (!story) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center space-y-8">
        <h1 className="text-4xl font-black uppercase italic">Story Manifestation Not Found</h1>
        <Link to="/stories" className="btn-minimal bg-white/5 px-8 py-4">Back to Showcase</Link>
      </div>
    );
  }
  ...
  return (
    <div className="pt-32 pb-40 px-6 max-w-5xl mx-auto">
      ...
      <Link to="/stories" className="inline-flex items-center gap-2 text-indigo-400 font-black uppercase tracking-widest italic mb-12 transition-all">
      ...
      >
        <div className="space-y-6">
          <div className="flex items-center gap-6 text-slate-500 text-sm font-black uppercase tracking-[0.3em] italic">
            <span className="flex items-center gap-2"><Calendar size={14} /> {story.timestamp.toDate().toLocaleDateString()}</span>
            <span className="flex items-center gap-2"><Clock size={14} /> {story.timestamp.toDate().toLocaleTimeString()}</span>
          </div>
          <h1 className="text-6xl md:text-8xl font-black text-white leading-[0.9] uppercase italic tracking-tighter">
            {story.title}
          </h1>
          <div className="flex items-center gap-4">
             <div className="w-10 h-[10px] bg-indigo-500" />
             <span className="text-indigo-400 font-bold uppercase tracking-widest">{story.author}</span>
          </div>
        </div>
        ...
        <div className="relative aspect-video rounded-3xl overflow-hidden border border-white/10 shadow-2xl group">
           ...
           <div className="absolute bottom-8 left-8 flex items-center gap-4">
              ...
              <span className="text-xs font-black uppercase tracking-[0.4em]">Cinematic Manifest Active</span>
           </div>
        </div>
