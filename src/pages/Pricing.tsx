...
export const Pricing = () => {
  ...
  const handleUpgrade = async (plan: PlanType) => {
    ...
    try {
      ...
      const paymentLink = plan === 'commands'
      ...
      <div className="content-grid gap-10">
        <div className="section-intro max-w-3xl text-center mx-auto">
          ...
          <p className="section-copy">
            All plans use the same premium purchase flow. You pay, complete the setup form, and receive a hosted starter site link.
          </p>
        </div>

        <div className="mx-auto inline-flex rounded-full border border-white/10 bg-white/[0.04] p-2 backdrop-blur-xl">
          {(['month', 'year'] as const).map((option) => (
            <button
              ...
              className={`rounded-full px-5 py-2.5 text-sm font-semibold transition ${
                cadence === option ? 'bg-indigo-500 text-white shadow-[0_0_24px_rgba(99,102,241,0.45)]' : 'text-slate-300 hover:text-white'
              }`}
            >
              ...
              >
                <div className="space-y-5">
                  <div className="flex items-center justify-between gap-4">
                    <div>
                      <div className="text-sm font-semibold uppercase tracking-[0.28em] text-indigo-200/90">{config.name}</div>
                      <h2 className="mt-2 text-4xl font-bold text-white">{getDisplayedPrice(plan)}</h2>
                    </div>
                ...
                <div className="space-y-3 text-sm text-slate-300">
                  ...
                  <div className="inline-flex items-center gap-3">
                    <CheckCircle2 className="h-4 w-4 text-emerald-300" />
                    {config.export ? 'GitHub export path included' : 'Hosted site delivery included'}
                  </div>
                  <div className="inline-flex items-center gap-3">
                    <CheckCircle2 className="h-4 w-4 text-emerald-300" />
                    {config.watermark ? 'Watermark stays on lower plans' : 'Watermark removed'}
                  </div>
                  <div className="inline-flex items-center gap-3">
                    <CheckCircle2 className="h-4 w-4 text-emerald-300" />
                    {plan === 'commands' ? 'No recurring billing' : cadenceCopy[cadence]}
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  ...
                  <p className="text-xs leading-5 text-slate-400">
                    All sales are final and non-refundable. After checkout you complete setup details before site generation starts.
                  </p>
                </div>
              </motion.div>
            );
        ...
        <div className="rounded-[32px] border border-white/10 bg-white/[0.04] p-6 text-sm leading-6 text-slate-300 backdrop-blur-xl">
          <h3 className="text-lg font-semibold text-white">What happens next</h3>
          <div className="mt-4 grid gap-4 md:grid-cols-3">
            <div className="rounded-2xl border border-white/10 bg-slate-950/60 p-4">
              <div className="font-semibold text-white">1. Secure checkout</div>
              <p className="mt-2">Stripe handles payment and returns you to the verified setup flow.</p>
            </div>
            <div className="rounded-2xl border border-white/10 bg-slate-950/60 p-4">
              <div className="font-semibold text-white">2. Short intake</div>
              <p className="mt-2">You enter your business description, industry, and style direction.</p>
            </div>
            <div className="rounded-2xl border border-white/10 bg-slate-950/60 p-4">
              <div className="font-semibold text-white">3. Hosted delivery</div>
              <p className="mt-2">The generator produces a live site link and stores the order for follow-up and support.</p>
            </div>
          </div>
        </div>
      </div>
