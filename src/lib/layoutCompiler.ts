...
function cssForVariant(variant: SiteVariant, industry: string, mediaOverride?: BrandAsset["media"]) {
...
function variantMarkup(variant: SiteVariant, brand: BrandAsset, prompt: string, industry: string) {
  ...
  const mediaTag =
    variant.id === "editorial"
      ? `<img src="${image}" alt="${escapeHtml(brand.name)} visual direction" loading="eager" />`
      : `<video src="${video}" autoplay muted loop playsinline preload="metadata" poster="${image}" aria-label="${escapeHtml(brand.name)} background video"></video>`;
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"/><meta name="viewport" content="width=device-width,initial-scale=1"/><title>${escapeHtml(brand.name)} | ${escapeHtml(variant.name)}</title><meta name="description" content="${escapeHtml(copy.subhead)}"/><style>${cssForVariant(variant, industry, brand.media)}.sound-toggle{position:fixed;left:18px;bottom:18px;z-index:20;border:1px solid rgba(255,255,255,.18);background:rgba(5,7,11,.72);color:white;border-radius:999px;padding:13px 16px;font-weight:850;backdrop-filter:blur(14px);box-shadow:0 18px 60px rgba(0,0,0,.34);cursor:pointer}.sound-toggle:hover{border-color:var(--a);transform:translateY(-2px)}</style></head><body>
    ...
    </main><button class="sound-toggle" type="button" id="soundToggle">Play ambient music</button><div class="vtw-watermark">Copyright © VoiceToWebsite.com</div><footer class="shell" style="padding:36px 0;color:rgba(255,255,255,.56)">Generated by VoiceToWebsite.com. Preview output is copyrighted and watermarked. Customer is responsible for final content, claims, permissions, privacy disclosures, and legal compliance.</footer>
    <script>
