...
function buildBloks(prompt: string, brand: BrandAsset): LayoutBlok[] {
  ...
  for (const type of sections) {
    ...
    if (type === "sidebar") {
      bloks.push({ component: "sidebar", order: order++, grid_span: 3, col_start: 1, ...copy, items: ["Overview", "Pages", "Leads", "Settings"] });
    } else if (type === "dashboard") {
      bloks.push({ component: "dashboard", order: order++, grid_span: 9, col_start: 4, ...copy, items: ["Live Preview", "Conversion Score", "Pending Tasks"] });
    } else {
      bloks.push({
        component: type,
        order: order++,
...
function renderBlok(blok: LayoutBlok, brand: BrandAsset) {
      ...
      <div class="rounded-3xl border border-gray-200 bg-white p-8 shadow-xl lg:p-14">
        ...
        <div class="grid items-center gap-10 lg:grid-cols-12">
          <div class="lg:col-span-7">
            <h1 class="mb-6 max-w-4xl text-4xl font-extrabold tracking-tight text-gray-950 md:text-6xl">${title}</h1>
            <p class="mb-8 text-lg leading-8 text-gray-600">${body}</p>
  ...
  if (blok.component === "sidebar") {
    return `<aside class="${base}"><div class="h-full rounded-2xl border border-gray-200 bg-white p-5 shadow-sm"><h2 class="mb-4 text-lg font-bold text-gray-950">${title}</h2><nav class="space-y-2">${items.map((item) => `<a href="#" class="flex items-center gap-3 rounded-lg px-3 py-2 text-sm font-medium text-gray-700 hover:bg-gray-100">${iconSvg("grid")}<span>${escapeHtml(item)}</span></a>`).join("")}</nav></div></aside>`;
  }

  if (blok.component === "dashboard") {
    return `<section class="${base}"><div class="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm"><div class="mb-6"><h2 class="text-3xl font-bold text-gray-950">${title}</h2><p class="mt-2 text-gray-600">${body}</p></div><div class="grid grid-cols-1 gap-4 md:grid-cols-3">${items.map((item, i) => `<article class="rounded-xl border border-gray-200 p-5"><div class="mb-3 text-blue-700">${iconSvg(i === 1 ? "chart" : "grid")}</div><h3 class="font-bold text-gray-950">${escapeHtml(item)}</h3><p class="mt-2 text-sm text-gray-600">Grid-aware app module placed inside the main col-span-9 workspace.</p></article>`).join("")}</div></div></section>`;
  }

  if (blok.component === "features" || blok.component === "services") {
    return `<section class="${base}"><div class="py-10"><div class="mx-auto mb-8 max-w-3xl text-center"><h2 class="mb-4 text-3xl font-extrabold tracking-tight text-gray-950 md:text-4xl">${title}</h2><p class="text-gray-600">${body}</p></div><div class="grid grid-cols-1 gap-8 md:grid-cols-3">${items.map((item, i) => `<article class="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm transition hover:-translate-y-1 hover:shadow-xl"><div class="mb-4 flex h-12 w-12 items-center justify-center rounded-xl bg-blue-100 text-blue-700">${iconSvg(i % 2 ? "spark" : "check")}</div><h3 class="mb-2 text-xl font-bold text-gray-950">${escapeHtml(item)}</h3><p class="text-gray-600">A non-default component populated from the request and locked to responsive grid placement.</p></article>`).join("")}</div></div></section>`;
  }

  if (blok.component === "pricing") {
    return `<section class="${base}"><div class="py-10"><div class="mx-auto mb-8 max-w-3xl text-center"><h2 class="mb-4 text-3xl font-extrabold text-gray-950 md:text-4xl">${title}</h2><p class="text-gray-600">${body}</p></div><div class="grid grid-cols-1 gap-6 md:grid-cols-3">${items.map((item, i) => `<div class="rounded-2xl border ${i === 1 ? "border-blue-600 ring-4 ring-blue-100" : "border-gray-200"} bg-white p-6 shadow-sm"><h3 class="text-xl font-bold text-gray-950">${escapeHtml(item)}</h3><div class="my-5 text-4xl font-extrabold text-gray-950">${i === 0 ? "$19" : i === 1 ? "$49" : "$99"}</div><ul class="mb-6 space-y-3 text-sm text-gray-600"><li>Grid compiled layout</li><li>Flowbite-ready UI</li><li>Responsive sections</li></ul><a class="block rounded-lg bg-blue-700 px-5 py-3 text-center text-sm font-medium text-white hover:bg-blue-800" href="#contact">${escapeHtml(blok.cta || "Choose plan")}</a></div>`).join("")}</div></div></section>`;
  }

  if (blok.component === "faq") {
    return `<section class="${base}"><div class="py-10"><h2 class="mb-6 text-3xl font-extrabold text-gray-950">${title}</h2><div id="accordion-flush" data-accordion="collapse" class="rounded-2xl border border-gray-200 bg-white p-4">${items.map((item, i) => `<h3 id="accordion-heading-${i}"><button type="button" class="flex w-full items-center justify-between border-b border-gray-200 py-5 text-left font-medium text-gray-700" data-accordion-target="#accordion-body-${i}" aria-expanded="${i === 0}" aria-controls="accordion-body-${i}"><span>${escapeHtml(item)}</span><svg data-accordion-icon class="h-3 w-3 shrink-0 rotate-180" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 10 6"><path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5 5 1 1 5"/></svg></button></h3><div id="accordion-body-${i}" class="${i === 0 ? "" : "hidden"}" aria-labelledby="accordion-heading-${i}"><div class="border-b border-gray-200 py-5 text-gray-600">${body}</div></div>`).join("")}</div></div></section>`;
  }

  if (blok.component === "contact") {
    return `<section id="contact" class="${base}"><div class="rounded-3xl bg-gray-950 p-8 text-white md:p-10"><div class="grid gap-8 md:grid-cols-2"><div><h2 class="mb-4 text-3xl font-extrabold">${title}</h2><p class="text-gray-300">${body}</p></div><form class="grid gap-4"><input class="rounded-lg border border-gray-700 bg-gray-900 px-4 py-3 text-white" placeholder="Name"/><input class="rounded-lg border border-gray-700 bg-gray-900 px-4 py-3 text-white" placeholder="Email"/><button class="rounded-lg bg-blue-600 px-5 py-3 font-medium hover:bg-blue-700" type="button">${iconSvg("mail")} ${escapeHtml(blok.cta || "Contact us")}</button></form></div></div></section>`;
  }

  return `<section class="${base}"><div class="rounded-2xl border border-gray-200 bg-white p-6"><h2 class="text-2xl font-bold text-gray-950">${title}</h2><p class="mt-2 text-gray-600">${body}</p></div></section>`;
}
