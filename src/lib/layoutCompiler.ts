...
const videoByIndustry: Record<string, string> = {
  ...
  medical: "https://cdn.coverr.co/videos/coverr-doctor-explaining-a-medical-procedure-9587/1080p.mp4",
  auto: "https://cdn.coverr.co/videos/coverr-driving-through-the-city-5722/1080p.mp4",
  music: "https://cdn.coverr.co/videos/coverr-dj-playing-music-5274/1080p.mp4",
  creative: "https://cdn.coverr.co/videos/coverr-artist-painting-on-canvas-9308/1080p.mp4",
...
const imageByIndustry: Record<string, string> = {
  ...
  medical: "https://images.unsplash.com/photo-1586773860418-d37222d8fce3?auto=format&fit=crop&w=1600&q=80",
  auto: "https://images.unsplash.com/photo-1503376780353-7e6692767b70?auto=format&fit=crop&w=1600&q=80",
  music: "https://images.unsplash.com/photo-1511379938547-c1f69419868d?auto=format&fit=crop&w=1600&q=80",
  creative: "https://images.unsplash.com/photo-1518005020951-eccb494ad742?auto=format&fit=crop&w=1600&q=80",
...
function detectIndustry(prompt: string) {
  ...
  if (/doctor|clinic|dental|health|medical|therapy|therapist/.test(p)) return "medical";
  if (/auto|car|mechanic|detailing|transport|driver/.test(p)) return "auto";
  if (/music|artist|band|dj|studio|producer|podcast/.test(p)) return "music";
  if (/design|agency|creative|photography|film|creator/.test(p)) return "creative";
...
function pickFromPrompt(prompt: string, fallback: string[]) {
  const quoted = [...prompt.matchAll(/["']([^"']{3,60})["']/g)].map((match) => match[1]);
  const listed = prompt
    .split(/\b(?:with|including|include|needs?|has|featuring)\b/i)
    .slice(1)
    .join(" ")
    .split(/,| and | plus |;/i)
    .map((item) => item.replace(/[^a-z0-9&\-\s]/gi, "").trim())
    .filter((item) => item.length > 3 && item.length < 42);
  return [...new Set([...quoted, ...listed, ...fallback])].slice(0, 4);
}

function serviceFallbacks(industry: string) {
  const map: Record<string, string[]> = {
    food: ["Signature menu", "Private events", "Online ordering", "Reservations"],
    fitness: ["Training plans", "Class schedule", "Coach profiles", "Memberships"],
    realestate: ["Featured listings", "Buyer consults", "Seller strategy", "Neighborhood guides"],
    beauty: ["Signature treatments", "Stylist booking", "Client care", "Membership packages"],
    law: ["Consultations", "Case review", "Practice areas", "Client resources"],
    medical: ["Appointments", "Patient services", "Care plans", "Insurance guidance"],
    auto: ["Service booking", "Detail packages", "Fleet support", "Estimate requests"],
    music: ["New releases", "Booking inquiries", "Tour dates", "Studio services"],
    creative: ["Portfolio direction", "Campaign design", "Production services", "Brand systems"],
    tech: ["Product demos", "Use cases", "Integrations", "Growth onboarding"],
    default: ["Signature offer", "Fast consultation", "Premium delivery", "Ongoing support"],
  };
  return map[industry] || map.default;
}

function ctaForIndustry(industry: string) {
  const map: Record<string, string> = {
    food: "Reserve or order",
    fitness: "Start training",
    realestate: "Book a showing",
    beauty: "Book an appointment",
    law: "Request a consultation",
    medical: "Schedule a visit",
    auto: "Get an estimate",
    music: "Book the experience",
...
function copyFor(brand: string, prompt: string, industry: string) {
  ...
  const services = pickFromPrompt(prompt, serviceFallbacks(industry));
  const cta = ctaForIndustry(industry);

  return {
    headline: `${brand} turns attention into action`,
    subhead: `A complete custom homepage generated for ${subject}, with design decisions matched to the brand, offer, audience, and conversion goal.`,
    intro: `Built from the prompt: "${prompt.slice(0, 150)}${prompt.length > 150 ? "..." : ""}"`,
    services,
    cta,
    proof: ["Brand-specific copy", "Responsive section system", "Conversion flow", "Relevant image, video, and sound"],
    testimonials: [
      `${brand} makes the offer feel obvious, premium, and ready for action from the first screen.`,
      "The page explains the business clearly, builds trust quickly, and gives visitors a reason to move.",
      "The visual direction feels custom, polished, and designed around the actual request.",
    ],
    faqs: [
      ...
      ["Does it include content?", "Yes. Headlines, service copy, proof points, pricing language, FAQ text, media direction, and contact copy are written into the output."],
    ],
  };
}
...
function cssForVariant(variant: SiteVariant, industry: string) {
  ...
  return `
    ...
    *{box-sizing:border-box}html{scroll-behavior:smooth}body{margin:0;background:#05070b;color:var(--ink);font-family:${variant.fontPair};overflow-x:hidden}
    ...
    .span-3{grid-column:span 3}.span-4{grid-column:span 4}.span-6{grid-column:span 6}.span-8{grid-column:span 8}.span-12{grid-column:span 12}.kpi{font-size:42px;font-weight:950;letter-spacing:-.05em}.muted{color:var(--muted)}.price{font-size:48px;font-weight:950;margin:14px 0}.gallery-img{height:330px;background:url("${media}") center/cover;border-radius:24px}.award{display:inline-flex;gap:8px;align-items:center;margin-top:18px;border:1px solid color-mix(in srgb,var(--a) 36%, transparent);background:color-mix(in srgb,var(--a) 12%, transparent);padding:10px 14px;border-radius:999px;color:white;font-weight:850;font-size:13px}.faq details{border-bottom:1px solid rgba(255,255,255,.12);padding:20px 0}.faq summary{cursor:pointer;font-weight:850;font-size:18px}.faq p{color:var(--muted);line-height:1.65}.contact{background:linear-gradient(135deg,color-mix(in srgb,var(--a) 18%, transparent),rgba(255,255,255,.04));border-radius:34px;padding:36px}.input{width:100%;margin-bottom:12px;border:1px solid rgba(255,255,255,.14);background:rgba(0,0,0,.24);border-radius:16px;padding:16px;color:white}
...
function variantMarkup(variant: SiteVariant, brand: BrandAsset, prompt: string, industry: string) {
  ...
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"/><meta name="viewport" content="width=device-width,initial-scale=1"/><title>${escapeHtml(brand.name)} | ${escapeHtml(variant.name)}</title><meta name="description" content="${escapeHtml(copy.subhead)}"/><style>${cssForVariant(variant, industry)}.sound-toggle{position:fixed;right:18px;bottom:18px;z-index:20;border:1px solid rgba(255,255,255,.18);background:rgba(5,7,11,.72);color:white;border-radius:999px;padding:13px 16px;font-weight:850;backdrop-filter:blur(14px);box-shadow:0 18px 60px rgba(0,0,0,.34);cursor:pointer}.sound-toggle:hover{border-color:var(--a);transform:translateY(-2px)}</style></head><body>
    <header class="nav"><div class="shell nav-inner"><div class="brand"><div class="mark">VTW</div><span>${escapeHtml(brand.name)}</span></div><nav><a href="#services">Services</a><a href="#pricing">Pricing</a><a href="#faq">FAQ</a><a class="btn" href="#contact">${copy.cta}</a></nav></div></header>
    <main>
      <section class="shell hero"><div class="reveal"><h1>${copy.headline}</h1><p>${copy.subhead}</p><div class="intro">${copy.intro}</div><a class="btn" href="#contact">${copy.cta}</a><div class="award">Award-caliber layout engine by VoiceToWebsite.com</div></div><div class="media float">${mediaTag}<div class="media-card"><strong>Live media direction</strong><p class="muted">Visuals are selected from copyright-safe open media sources and matched to the page subject.</p></div></div></section>
      <section class="shell"><div class="grid"><div class="card span-3"><div class="kpi">01</div><p class="muted">Custom page voice, brand message, and audience-specific copy.</p></div><div class="card span-3"><div class="kpi">02</div><p class="muted">Responsive sections with premium motion and hover lighting.</p></div><div class="card span-3"><div class="kpi">03</div><p class="muted">Relevant image, video, wallpaper, and ambient sound system.</p></div><div class="card span-3"><div class="kpi">04</div><p class="muted">Lead-ready conversion flow with clear next actions.</p></div></div></section>
    ...
    </main><button class="sound-toggle" type="button" id="soundToggle">Play ambient music</button><footer class="shell" style="padding:36px 0;color:rgba(255,255,255,.56)">Generated by VoiceToWebsite.com. Media from free-to-use open web sources; replace with final licensed assets before regulated ad campaigns.</footer>
    <script>
