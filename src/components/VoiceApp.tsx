...
import { PlanType, PLAN_LIMITS } from '@/constants/plans';
...
export const VoiceApp = ({ 
  ...
  const handleGenerate = async (text: string = input) => {
    ...
    if (generationCount >= limits.commands) {
      setGenError(`Neural limit reached for ${limits.name}. Upgrade to Pro for continued architectural access.`);
      setShowPaywall(true);
      return;
    }
  ...
  return (
    <div className="w-full space-y-12">
      ...
      <div className="glass-blur p-8 md:p-12 brutal-shadow border-t border-white/10 relative overflow-hidden">
        <div className="absolute top-0 left-0 w-full h-[2px] bg-gradient-to-r from-transparent via-indigo-600 to-transparent" />
        
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center relative z-10">
          <div className="lg:col-span-8 space-y-8">
            <div className="flex items-center gap-4">
              <div className={`w-12 h-12 rounded-full flex items-center justify-center transition-all ${isListening ? 'bg-red-500 animate-pulse shadow-[0_0_20px_rgba(239,68,68,0.5)]' : 'bg-white/5 border border-white/10'}`}>
                <Volume2 size={24} className={isListening ? 'text-white' : 'text-slate-500'} />
              </div>
              <div className="space-y-1">
                <h3 className="text-sm font-black uppercase tracking-[0.2em] italic">Vocal Sync Active</h3>
                <p className="text-[10px] text-slate-500 uppercase font-bold tracking-widest italic leading-none">
                  Neural Power: <span className={generationCount >= limits.commands ? 'text-red-500' : 'text-emerald-500'}>{limits.commands - generationCount} Command{limits.commands - generationCount !== 1 ? 's' : ''} Remaining</span>
                </p>
              </div>
            </div>
            ...
            <div className="relative group">
              ...
              <div className="absolute right-2 md:right-4 top-1/2 -translate-y-1/2 flex items-center gap-4">
                ...
                <FizzyButton 
                  ...
                  onClick={() => !isGenerating && input.trim() && handleGenerate()}
            ...
            <div className="flex flex-wrap gap-4 items-center">
              <span className="text-[10px] font-black uppercase text-indigo-500 tracking-widest italic flex items-center gap-2">Strategic Protocols:</span>
              <div className="flex flex-wrap gap-3">
                {['Launch Empire Store', 'Add Pro Paywall', 'Neural Layout', 'Optimize ROI'].map((hint) => (
                  <button 
                    key={hint} 
                    onClick={() => { playTick(); setInput(hint); }}
            ...
            {genError && (
              ...
              >
                ...
                <span className="text-[10px] font-black uppercase text-red-500 tracking-widest italic">Neural Sync Failed: {genError}</span>
                <button onClick={() => setGenError(null)} className="ml-auto text-red-500/40 hover:text-red-500">
                  <X size={14} />
                </button>
              </motion.div>
            )}
          ...
          <div className="lg:col-span-4 flex items-center justify-center h-48 lg:h-64">
            <motion.div 
              ...
              onClick={toggleListening}
            >
              <div className={`absolute inset-0 rounded-full border border-indigo-600/10 ${isListening ? 'animate-ping' : ''}`} />
          ...
          >
            ...
            <div 
              className={`mx-auto transition-all duration-700 metal-gothic-border group relative ${
                ...
                viewMode === 'tablet' ? 'w-full max-w-[768px] h-[800px]' : 
            ...
            >
                ...
                {isGenerating && (
                  ...
                  >
                    ...
                    <div className="absolute inset-0 flex flex-col items-center justify-center space-y-4">
                      ...
                      <span className="text-[10px] font-black uppercase tracking-[0.5em] text-white italic ultra-glow">Building Digital Reality...</span>
                    </div>
                  </motion.div>
                )}
              </AnimatePresence>
              ...
              {/* Scanline Overlay */}
              <div className="absolute inset-0 pointer-events-none bg-[linear-gradient(to_bottom,transparent_0%,rgba(79,70,229,0.02)_50%,transparent_100%)] bg-[length:100%_4px] opacity-20" />
            </div>
          </motion.div>
        ) : (
          <div className="aspect-video bg-black rounded-3xl overflow-hidden relative group/preview">
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center gap-8 text-center p-12">
              ...
              <div className="space-y-4">
                <p className="text-xl font-black uppercase italic tracking-tighter text-white ultra-glow">Neutral Output Awaiting Vector Input</p>
                <p className="text-[10px] text-indigo-400 font-bold uppercase tracking-[0.5em]">Speak to Launch Reality</p>
              </div>
            </div>
          </div>
        )}
        ...
        {showPaywall && (
          ...
          >
            <div className="max-w-md w-full glass-premium p-12 text-center space-y-8 brutal-shadow relative overflow-hidden">
               ...
               <div className="space-y-4">
                 <h2 className="text-4xl font-black uppercase italic tracking-tighter">🔥 You Hit The Limit</h2>
                 <p className="text-slate-400 font-light italic">Unlock unlimited websites, elite neural building, and high-conversion monetization tools.</p>
               </div>
