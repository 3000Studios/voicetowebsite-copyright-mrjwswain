        ...
        {letters.map((char, i) => (
          <motion.span
            ...
            animate={{
              ...
              color: [colors[i % colors.length], colors[(i+1) % colors.length], colors[i % colors.length]],
            }}
            transition={{
              y: { 
                type: "spring",
      ...
      <AnimatePresence>
        {phase === 'melting' && (
          <motion.div
            initial={{ height: 0 }}
            animate={{ height: "10vh" }}
            className="absolute bottom-0 left-0 w-full bg-gradient-to-t from-indigo-600 via-cyan-600 to-transparent blur-xl opacity-50"
          />
        )}
      </AnimatePresence>
    </motion.div>
