...
function verifyAdminUi() {
  ...
  const hasGuard = (content) =>
    content.includes("access-guard.js") && (content.includes('type="module"') || content.includes("type='module'"));

  const mustHaveIds = {
    "voice-commands.html": ["startBtn", "stopBtn", "shipButton", "clearHistory", "commandHistory"],
    "customer-chat.html": ["sessions", "messages", "reply", "sendReply", "refreshSessions"],
    ...
    if (ids) {
      for (const id of ids) {
        if (!content.includes(`id="${id}"`) && !content.includes(`id='${id}'`)) {
          throw new Error(`Admin page missing expected element id="${id}": admin/${file}`);
        }
      }
    }
  }
...
function verifyCatalog() {
  ...
  const opsCatalogPath = path.join(process.cwd(), "ops", "site", "products.json");
  const publicCatalogPath = path.join(process.cwd(), "public", "config", "products.json");
  if (!fs.existsSync(rootCatalogPath)) throw new Error("products.json missing");
  if (!fs.existsSync(opsCatalogPath)) throw new Error("ops/site/products.json missing");
  if (!fs.existsSync(publicCatalogPath)) throw new Error("public/config/products.json missing");
  ...
  if (stableStringify(rootCatalog) !== stableStringify(publicCatalog)) {
    throw new Error("public/config/products.json is out of sync with products.json");
  }

  const items = [...(rootCatalog.products || []), ...(rootCatalog.apps || []), ...(rootCatalog.subscriptions || [])];
  ...
  items.forEach((item) => {
    if (!item || typeof item !== "object") throw new Error("Catalog item is not an object");
    ...
    if (item.currency && String(item.currency).toUpperCase() !== "USD") {
      throw new Error(`Catalog item ${item.id} currency must be USD or omitted`);
    }

    const previewUrl = String(item.previewUrl || "").trim();
    if (previewUrl) {
      if (!previewUrl.startsWith("/") || !previewUrl.endsWith(".html")) {
        throw new Error(`Catalog item ${item.id} has invalid previewUrl: ${previewUrl}`);
      }
    ...
    if (downloadUrl) {
      if (!downloadUrl.startsWith("/downloads/")) {
        throw new Error(`Catalog item ${item.id} has invalid downloadUrl: ${downloadUrl}`);
      }
      const filePath = path.join(process.cwd(), "public", downloadUrl.replace(/^\//, ""));
