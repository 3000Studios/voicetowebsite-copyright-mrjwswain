...
export async function handlePublishRequest({ request, env }) {
  ...
  const uploadResults = await Promise.all(
    assets.map(async (asset) => {
      ...
      return { asset, bufLength: buf.byteLength };
    })
  );

  const statements = uploadResults.map(({ asset, bufLength }) =>
    env.D1.prepare("INSERT INTO site_assets (site_id, kind, r2_key, content_type, size_bytes) VALUES (?,?,?,?,?)").bind(
      siteId,
      asset.kind,
      asset.key,
      asset.contentType,
  ...
  statements.push(env.D1.prepare("UPDATE sites SET status = ? WHERE id = ?").bind("published", siteId));
