...
export const Dashboard = () => {
  ...
  const [activeTab, setActiveTab] = useState<'builder' | 'sites' | 'publish'>('builder');
  ...
  const handleSave = async () => {
    ...
    if (userSites.length >= limits.sites) {
      setSaveError(`Persistence limit reached for ${limits.name}. Upgrade to create more sites.`);
      return;
    }
  ...
  if (!isReady || !user) return (
    <div className="min-h-screen bg-[#050505] flex items-center justify-center">
      <div className="flex flex-col items-center gap-6">
        ...
        <span className="text-[10px] font-black uppercase text-indigo-500 tracking-[0.5em] italic">Initializing Neural Link...</span>
      </div>
    </div>
  );
      ...
      <div className="min-h-screen bg-black flex items-center justify-center p-6 lg:p-24 overflow-hidden relative">
        ...
        >
          <div className="space-y-6">
            ...
            <div className="h-px w-full bg-gradient-to-r from-transparent via-indigo-600 to-transparent my-12" />
            <h3 className="text-2xl md:text-3xl font-black uppercase tracking-widest italic text-white/80">For subscribers use only.</h3>
            <p className="text-lg md:text-xl text-slate-400 font-light italic leading-relaxed max-w-2xl mx-auto">
              Don’t be left out wondering what the dashboard looks like! Our builder engine is reserved for elite operators who have committed to their digital empire. 
              <span className="text-indigo-400 block mt-4 font-bold">Subscribe now and get instant access to your command center.</span>
            </p>
          </div>
        ...
        <div className="flex flex-col lg:flex-row items-start lg:items-end justify-between mb-16 gap-12 border-b border-white/5 pb-16">
          <div className="space-y-6">
            ...
            <div className="flex flex-wrap items-center gap-8 text-slate-500 font-black uppercase text-[10px] tracking-[0.3em] italic">
              <span className="flex items-center gap-2">Operator: <span className="text-indigo-400">{user.profile?.username || user.displayName}</span></span>
              <span className="flex items-center gap-2">Protocol: <span className="text-white px-2 py-0.5 bg-indigo-500/20 border border-indigo-500/40">{limits.name}</span></span>
              <span className="flex items-center gap-2">Frequency: <span className="text-emerald-500 animate-pulse">Sync Active</span></span>
            </div>
          </div>
          ...
          <div className="flex bg-white/5 p-1 brutal-shadow border border-white/10">
            {(['builder', 'sites', 'publish'] as const).map((tab) => (
              ...
              >
                {tab === 'builder' ? 'Voice Engine' : tab === 'sites' ? 'My Sites' : 'Deployment'}
              </button>
            ))}
          </div>
        </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
                ...
                <div className="lg:col-span-3 space-y-8">
                  <div className="glass-blur p-8 brutal-shadow space-y-8 border-l-4 border-indigo-600">
                    <div className="space-y-2">
                      <h3 className="text-xs font-black uppercase tracking-widest text-indigo-500 italic">Neural Action</h3>
                      <p className="text-[10px] text-slate-500 font-medium italic">Execute structural modifications via voice.</p>
                    </div>
                    ...
                    <div className="pt-8 border-t border-white/5 space-y-4">
                      <h4 className="text-[9px] font-black uppercase tracking-widest text-slate-600 italic">Quick Protocols</h4>
                      <div className="flex flex-col gap-2">
                        {['Dark Mode Synthesis', 'Bento Grid Injection', 'Mobile Optimization'].map((proto) => (
                          <button key={proto} className="w-full py-3 px-4 bg-black/40 border border-white/5 text-left text-[9px] font-black uppercase tracking-widest text-slate-500 hover:border-indigo-500 hover:text-white transition-all italic">
                            {proto}
                          </button>
                        ))}
                      </div>
                  ...
                  <div className="glass-blur p-8 space-y-6">
                    ...
                    <div className="space-y-4">
                      <div className="flex justify-between items-center">
                        <span className="text-[8px] font-black uppercase tracking-widest text-slate-500 italic">Render Latency</span>
                        <span className="text-[8px] font-mono text-emerald-500">2ms</span>
                      </div>
                      <div className="w-full h-1 bg-white/5 overflow-hidden">
                        <motion.div animate={{ x: [-100, 100] }} transition={{ repeat: Infinity, duration: 2 }} className="w-1/2 h-full bg-indigo-500/50" />
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="lg:col-span-9 relative">
                  ...
                  <div className="relative z-10 glass-blur p-2 brutal-shadow">
                    <VoiceApp isEditing={true} existingHtml={currentHtml} onUpdateHtml={setCurrentHtml} />
                  </div>
                </div>
              </div>
            )}
            ...
            {activeTab === 'sites' && (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-12">
                <motion.div 
                  ...
                  className="aspect-video bg-white/5 border-2 border-dashed border-white/10 flex flex-col items-center justify-center gap-6 cursor-pointer hover:border-indigo-500 hover:bg-white/[0.08] transition-all group p-12"
                >
                  <div className="w-20 h-20 rounded-full border border-white/10 flex items-center justify-center group-hover:bg-indigo-600 group-hover:border-indigo-600 transition-all duration-500">
                    <Plus className="text-slate-500 group-hover:text-white" size={32} />
                  </div>
                  <div className="text-center space-y-2">
                    <span className="text-[10px] font-black uppercase tracking-[0.4em] text-white italic block">Create New Identity</span>
                    <span className="text-[8px] font-medium text-slate-500 uppercase tracking-widest">Costs 10 Credits</span>
                  </div>
                </motion.div>
                ...
                {userSites.map((site) => (
                  ...
                  >
                    <div className="aspect-video bg-slate-900 border-b border-white/5 relative overflow-hidden flex items-center justify-center p-8">
                      <LayoutTemplate size={48} className="text-white/10 absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 group-hover:scale-150 transition-transform duration-700" />
                      <div className="relative z-10 flex flex-col items-center gap-4 opacity-0 group-hover:opacity-100 transition-all duration-500 translate-y-10 group-hover:translate-y-0">
                        <button 
                          onClick={() => { setCurrentHtml(site.html); setActiveTab('builder'); }}
                    ...
                    <div className="p-8 space-y-6">
                      <div className="flex items-center justify-between">
                        <h4 className="text-2xl font-black uppercase tracking-tighter italic text-white group-hover:text-indigo-400 transition-colors">{site.title || 'Untitled-Protocol'}</h4>
                        <span className="px-3 py-1 bg-white/10 text-[9px] font-black uppercase tracking-widest text-slate-400">Stable</span>
                      </div>
                      <div className="flex items-center justify-between text-[9px] font-black uppercase tracking-widest text-slate-600 border-t border-white/5 pt-6">
                        <span className="flex items-center gap-2"><Clock size={12} className="text-indigo-500" /> {site.timestamp?.toDate().toLocaleDateString()}</span>
                        <span className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity text-emerald-500">Sync Correct</span>
                      </div>
                    </div>
                  </motion.div>
                ))}
            ...
            {activeTab === 'publish' && (
              <div className="max-w-4xl mx-auto space-y-16 py-12">
                <div className="text-center space-y-6">
                  <span className="text-indigo-500 font-mono text-xs uppercase tracking-[0.5em] font-black">Digital Authority</span>
                  <h3 className="text-4xl md:text-6xl font-black uppercase tracking-tighter italic leading-none">Domain <span className="text-indigo-500">Buyout</span></h3>
                  <p className="text-xl text-slate-500 font-medium italic max-w-2xl mx-auto">Connect your launched visions to a permanent address on the global grid.</p>
                </div>
                ...
                <div className="glass-blur p-16 space-y-12 brutal-shadow relative overflow-hidden">
                  ...
                  <div className="space-y-6">
                    <label className="text-[10px] font-black uppercase tracking-[0.4em] text-slate-500 italic block">Proposed Neural Domain</label>
                    <div className="flex flex-col sm:flex-row gap-4">
                      ...
                      />
                      <button 
                        onClick={() => window.open(`https://www.namecheap.com/domains/registration/results/?domain=${domainSearch}`, '_blank')}
                  ...
                  <div className="pt-12 grid grid-cols-1 md:grid-cols-2 gap-8 text-center">
                    <div className="p-10 border border-white/5 hover:border-indigo-500 transition-all space-y-4 group">
                      <Cloud className="text-indigo-500 mx-auto group-hover:scale-110 transition-transform" size={40} />
                      <h4 className="text-xl font-black uppercase italic tracking-tighter">Fast-Sync Relay</h4>
                      <p className="text-[10px] text-slate-600 uppercase font-bold tracking-widest italic leading-relaxed">Instantly route your site through our high-speed edge proxy.</p>
                      <button className="w-full py-4 border border-white/10 text-white font-black uppercase tracking-widest text-[9px] hover:bg-white hover:text-black transition-all">Initialize Relay</button>
                    </div>
                    <div className="p-10 border border-white/5 hover:border-indigo-500 transition-all space-y-4 group">
                      <Search className="text-indigo-500 mx-auto group-hover:scale-110 transition-transform" size={40} />
                      <h4 className="text-xl font-black uppercase italic tracking-tighter">Premium Buyout</h4>
                      <p className="text-[10px] text-slate-600 uppercase font-bold tracking-widest italic leading-relaxed">Secure domain ownership via Namecheap integration.</p>
                      <a 
