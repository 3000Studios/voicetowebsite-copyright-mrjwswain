...
import { Aura3DIcon } from './Aura3DIcon';

export const AICompanion = () => {
  const [isOpen, setIsOpen] = useState(false);
  const [message, setMessage] = useState('');
  const [chat, setChat] = useState<{ role: 'user' | 'ai'; text: string }[]>([
    { role: 'ai', text: "Hey there! I'm Aura. I build websites faster than you can say 'Success'. What's your empire about?" }
  ]);
    ...
    const loadVoices = () => {
      if ('speechSynthesis' in window) {
        ...
        if (voices.length > 0) {
          ...
          console.log("Aura initialized with voice:", femaleVoice?.name || "Default");
        }
      }
    };
  ...
  const handleSend = () => {
    ...
    setTimeout(() => {
      const response = funnyLines[Math.floor(Math.random() * funnyLines.length)];
          ...
          >
            <div className="p-3 bg-indigo-600/20 flex items-center gap-3 border-b border-white/5">
              <div className="w-8 h-8 rounded-full overflow-hidden border border-indigo-500 bg-black relative">
                 ...
                 />
                 <div className={`absolute bottom-0 right-0 w-2 h-2 rounded-full bg-emerald-500 border border-black ${isSpeaking ? 'animate-pulse' : ''}`} />
              </div>
              <div className="flex-1">
                <h4 className="text-[10px] font-black uppercase italic tracking-widest text-white">Aura</h4>
                <p className="text-[8px] text-indigo-400 font-bold uppercase tracking-widest">Synth Engine</p>
              </div>
              <button onClick={() => setMute(!mute)}>
                {mute ? <VolumeX size={14} /> : <Volume2 size={14} className="text-indigo-400" />}
              </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-3 no-scrollbar bg-black/40">
              {chat.map((m, i) => (
                <div key={i} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                  <div className={`max-w-[85%] p-3 rounded-xl text-[10px] font-medium italic leading-relaxed ${m.role === 'user' ? 'bg-indigo-600 text-white rounded-tr-none' : 'bg-white/5 text-slate-200 rounded-tl-none border border-white/5'}`}>
                    {m.text}
                  </div>
                </div>
              ))}
