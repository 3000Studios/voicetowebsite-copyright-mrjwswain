...
const openPayPalModal = async (product) => {
  ...
  modal.innerHTML = `
    <div class="vt-pay-shell">
      <button class="vt-pay-close" type="button" aria-label="Close">×</button>
  ...
  window.paypal
    .Buttons({
      ...
      onApprove: async (_data, actions) => {
        ...
        modal.querySelector(".vt-pay-note").textContent = "Payment successful. Thank you.";
...
const wireBuyButtons = () => {
  document.addEventListener("click", (e) => {
    ...
    const product = loadProducts().find((p) => String(p.id || "") === id) || null;
    if (!product) {
