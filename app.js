...
const resolvePlacementTarget = (hint) => {
  ...
  const selectors = [
    custom && (custom.startsWith('#') || custom.startsWith('.')) ? custom : null,
    preferred && (preferred.startsWith('#') || preferred.startsWith('.')) ? preferred : null,
    `#${normalized}`,
    `.${normalized}`,
  ].filter(Boolean);
...
const detectMediaType = (url, hint = '') => {
  ...
  if (lowered.includes('audio') || lowered.includes('song') || lowered.includes('music') || lowered.includes('podcast')) return 'audio';
  if (lowered.includes('video')) return 'video';
  const cleanUrl = url?.toLowerCase() || '';
  if (cleanUrl.includes('youtube.com') || cleanUrl.includes('youtu.be') || cleanUrl.includes('vimeo.com') || cleanUrl.includes('spotify.com')) return 'embed';
...
const renderSearchResults = (results, placementHint) => {
  ...
  results.forEach((result) => {
    ...
    addBtn.addEventListener('click', () => addCardToPlacement({ ...result, badge: 'Search' }, placementHint));
    const playBtn = document.createElement('button');
    playBtn.className = 'ghost';
    playBtn.textContent = 'Play';
    playBtn.addEventListener('click', () => handleMediaRequest(result.title, 'video', placementHint));
...
const handleMediaRequest = async (query, typeHint = '', placementHint) => {
  ...
  if (urlMatch) {
    ...
    enqueueMedia(item);
    playMedia(item);
    addCardToPlacement({ title, description: url, link: url, badge: type === 'audio' ? 'Audio' : 'Media' }, placementHint);
    return;
  }

  const wantsAudio = /audio|song|music|podcast/i.test(typeHint) || /audio|song|music|podcast/i.test(query);
  if (wantsAudio) {
    try {
  ...
  enqueueMedia(item);
  playMedia(item);
  addCardToPlacement({ title: query, description: 'Video search playlist', link: embedUrl, badge: 'Video' }, placementHint);
};
...
const applyTextEffect = (targetKey, effectString = '') => {
  ...
  if (!targets.length && selectorMatch) {
    document.querySelectorAll(selectorMatch[0]).forEach((el) => targets.push(el));
  }
  if (!targets.length && elements.headline) targets.push(elements.headline);

  const effectEntry = Object.entries(textEffects).find(([keyword]) => effectString.toLowerCase().includes(keyword));
...
const handleAdvancedCommand = async (command) => {
  ...
  const mediaMatch = command.match(/play\s+(?:a\s+)?(video|song|audio|music|podcast)?\s*(?:about|for)?\s*(.+)/i);
  if (mediaMatch && mediaMatch[2]) {
    await handleMediaRequest(mediaMatch[2], mediaMatch[1] || '', placementHint);
    return;
  }
  const styleMatch = command.match(/(?:style|animate|make)\s+(headline|subhead|cta|text|title)\s+(.*)/i);
  if (styleMatch) {
    const targetKey = styleMatch[1] === 'title' ? 'headline' : styleMatch[1];
    applyTextEffect(targetKey, styleMatch[2]);
    return;
  }
  const addSectionMatch = command.match(/(?:add|create|drop|place)\s+(?:a\s+)?(?:section|block|card)\s+(.*)/i);
  if (addSectionMatch) {
    addCardToPlacement(
      { title: addSectionMatch[1] || 'New section', description: command, badge: 'Section' },
      placementHint
    );
    return;
  }
  if ((command.toLowerCase().includes('place it') || command.toLowerCase().includes('add it')) && lastSearchResults.length) {
    addCardToPlacement({ ...lastSearchResults[0], badge: 'Search' }, placementHint);
  }
};
...
const init = () => {
  ...
  if (runButton) {
    runButton.addEventListener('click', () => {
      runTextCommand().catch((err) => logTranscript(`Command error: ${err.message}`));
    });
  }
