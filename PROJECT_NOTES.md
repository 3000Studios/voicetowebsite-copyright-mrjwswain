...
- Domain/routes: `voicetowebsite.com/*`, `www.voicetowebsite.com/*` (set in `wrangler.toml`). Old domain `youtuneai.com/*` can be redirected at Cloudflare if desired.
- Worker entry: `worker.js` imports `functions/orchestrator.js` for `/api/orchestrator`; serves static assets from `dist`.
...
- **Always Accept All**: Use `git add .` and `git commit` to accept all changes automatically in scripts. Avoid interactive prompts.
