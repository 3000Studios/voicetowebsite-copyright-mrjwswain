...
const SECURITY_HEADERS = {
  "Content-Security-Policy": `
    default-src 'self';
    script-src 'self' 'nonce-{nonce}' https://cdn.tailwindcss.com https://cdnjs.cloudflare.com https://www.googletagmanager.com https://pagead2.googlesyndication.com https://googleads.g.doubleclick.net https://www.google-analytics.com https://accounts.google.com https://js.stripe.com https://www.paypal.com https://esm.sh https://static.cloudflareinsights.com;
    style-src 'self' 'unsafe-inline' data: https://fonts.googleapis.com;
    font-src 'self' data: https://fonts.gstatic.com;
    img-src 'self' data: https:;
    connect-src 'self' https://www.google-analytics.com https://accounts.google.com https://api-m.paypal.com https://api-m.sandbox.paypal.com https://www.paypal.com https://js.stripe.com https://cloudflareinsights.com;
    frame-src https://accounts.google.com https://googleads.g.doubleclick.net https://tpc.googlesyndication.com https://www.youtube.com https://www.paypal.com https://js.stripe.com;
    object-src 'none';
    base-uri 'self';
    form-action 'self';
...
const getCacheControlForPath = (pathname, env) => {
  ...
  const htmlLike =
    ...
    !normalized.includes(".");
  if (staticByExt.test(normalized)) {
    return (
  ...
  if (htmlLike) {
    return (
      String(env?.CACHE_CONTROL_HTML || "").trim() || "public, max-age=600"
    );
  }
  return (
    String(env?.CACHE_CONTROL_STATIC || "").trim() ||
...
const addSecurityHeaders = (response, options = {}) => {
  ...
  if (options.cacheControl) {
    ...
    headers.set(
      "Cache-Control",
      getCacheControlForPath(options.pathname, options.env)
    );
  }
...
const ADMIN_SHELL_INLINE_CSS = `:root {
  --ccos-bg: #07111f;
  --ccos-panel: rgba(8, 18, 33, 0.84);
  --ccos-panel-2: rgba(14, 28, 49, 0.88);
  --ccos-border: rgba(148, 163, 184, 0.22);
  --ccos-text: #e2e8f0;
  --ccos-muted: #93a4bb;
  --ccos-accent: #14b8a6;
  --ccos-accent-2: #3b82f6;
  --ccos-danger: #ef4444;
  --ccos-warning: #f59e0b;
  --ccos-ok: #22c55e;
}
...
body.ccos-body {
  ...
  background:
    ...
    radial-gradient(circle at 90% 8%, rgba(59, 130, 246, 0.2), transparent 30%),
    linear-gradient(160deg, #04101f 0%, #020913 58%, #020811 100%);
}
...
#sidebar {
  border-right: 1px solid var(--ccos-border);
...
.ccos-nav-item:hover,
.ccos-nav-item.is-active {
  border-color: rgba(20, 184, 166, 0.8);
  box-shadow: 0 0 0 1px rgba(20, 184, 166, 0.3) inset;
...
.ccos-top-actions input,
.ccos-top-actions button,
.ccos-input,
.ccos-button,
.ccos-select,
.ccos-textarea {
  border: 1px solid var(--ccos-border);
  background: var(--ccos-panel-2);
  color: var(--ccos-text);
...
.ccos-top-actions button:disabled,
.ccos-button:disabled {
  cursor: not-allowed;
  opacity: 0.6;
}
...
.ccos-pill.warn {
  color: var(--ccos-warning);
}

.ccos-pill.err {
  color: var(--ccos-danger);
}
...
<html lang="en">
  ...
  <body>
    ...
    <script src="/admin/access.js" type="module"></script>
  </body>
</html>`;
};
...
<html lang="en">
  <head>
    ...
    <style>${ADMIN_SHELL_INLINE_CSS}</style>
...
const buildAutoContentPage = ({ url, path }) => {
  const slug = String(path || "/")
    .replace(/^\/+/, "")
    .replace(/\/+$/, "")
    .replace(/\.html$/i, "");
  const topic = (slug || "home")
    .split(/[\/_-]+/)
    .filter(Boolean)
    .map((part) => part.charAt(0).toUpperCase() + part.slice(1))
    .join(" ");
  const title = topic ? `${topic} | VoiceToWebsite` : "VoiceToWebsite";
  const daySeed = getDailySeed();
  const description = `Daily-updated ${topic || "website"} insights, resources, and implementation guidance generated by VoiceToWebsite.`;
  const canonical =
    `${url.origin}/${slug}`.replace(/\/+$/, "") || `${url.origin}/`;
  return `<!doctype html>
<html lang="en">
  <head>
    <meta charset="UTF-8" />
    ...
    <meta property="og:image" content="${url.origin}/vtw-wallpaper.png" />
    <meta name="twitter:card" content="summary_large_image" />
    <meta name="twitter:title" content="${title}" />
    <meta name="twitter:description" content="${description}" />
    <meta name="twitter:image" content="${url.origin}/vtw-wallpaper.png" />
    <link rel="stylesheet" href="/styles.css" />
  </head>
  <body>
    <main style="max-width:960px;margin:64px auto;padding:24px">
      <h1>${topic || "VoiceToWebsite"}</h1>
      <p>This page is auto-generated and refreshed daily to maintain complete site coverage and SEO continuity.</p>
      <p><strong>Daily seed:</strong> ${daySeed}</p>
      <section>
        <h2>What You Can Do Here</h2>
        <ul>
          <li>Explore curated guides related to ${topic || "this topic"}.</li>
          <li>Discover implementation examples and conversion-ready patterns.</li>
          <li>Navigate to core pages: <a href="/">Home</a>, <a href="/store">Store</a>, <a href="/pricing">Pricing</a>, <a href="/contact">Contact</a>.</li>
        </ul>
      </section>
    </main>
  </body>
  ...
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      if (url.pathname === "/api/demo/save" && request.method === "POST") {
        try {
          ...
          const previewUrl = new URL(
            String(
              generateData.previewUrl || `/preview/${generateData.siteId}`
            ),
            origin
          ).toString();
      ...
      if (url.pathname === "/api/chat" && request.method === "POST") {
        return addSecurityHeaders(
          await handleChatRequest({ request, env, ctx })
        );
      }
      ...
      if (url.pathname === "/api/license/verify" && request.method === "POST") {
        try {
          ...
          if (!license)
            return jsonResponse(400, { error: "License required." });
            ...
            try {
              ...
              if (relatedOrderId) {
                ...
                await env.D1.prepare(
                  ...
                  .bind(
                    String(resourceStatus || "webhook").toLowerCase(),
                    rowId
                  )
                  .run();
              }
      ...
      if (contentType.includes("text/html")) {
        ...
        <script nonce="${cspNonce}">
          window.__ENV = {
            ...
            ADSENSE_PUBLISHER: "${env.ADSENSE_PUBLISHER || env.NEXT_PUBLIC_ADSENSE_PUBLISHER_ID || getAdSenseClientId(env)}",
            ADSENSE_CUSTOMER_ID: "${env.ADSENSE_CUSTOMER_ID || ""}",
      ...
      `;
        ...
        const globalSynthInjection = `
        <link rel="preload" as="style" href="/synth-field.css" />
        <link rel="stylesheet" href="/synth-field.css" />
        <link rel="preload" as="script" href="/synth-field.js" />
        <script src="/synth-field.js" defer></script>
      `;
        ...
        const injected = text
          ...
          .replace(
            "</head>",
            `${envInjection}\n${globalSynthInjection}\n</head>`
          ); // Inject variables and global immersive layer early
        ...
        if (!skipHeavyInjection) {
          ...
          const hasTwitterCard =
            /<meta\b[^>]*(?:name|property)=["']twitter:card["']/i.test(
              seoInjected
            );
      ...
      `;
          ...
          withAdsense = shouldInjectAdsense
        ...
        const nonceHardenedHtml = withAdsense.replace(
        ...
        headers.set("Cache-Control", getCacheControlForPath(pathname, env));
        headers.delete("Pragma");
        headers.set("X-Robots-Tag", robotsTag);

        return addSecurityHeaders(
          new Response(nonceHardenedHtml, {
            status: assetRes.status,
            headers,
          }),
          { cspNonce, pathname, env }
        );
      }
      return addSecurityHeaders(assetRes, { pathname, env });
    } catch (err) {
