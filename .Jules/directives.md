...
- **No secrets in client code**: never inject passwords/signing keys into `window.__ENV` (see
  `.Jules/sentinel.md`).
- **Keyboard-first gatekeeping**: any “Enter / Start / Ignite” interaction must be reachable and
  usable via keyboard (see `.Jules/palette.md`).
- **Focus visibility**: strong `:focus-visible` styles everywhere (especially if custom cursor /
  heavy motion exists).
...
- **One design language**: shared tokens (color, spacing, typography), consistent components
  (buttons, cards, tables).
- **Performance**: avoid layout shift, reduce JS-driven animation where CSS can do it, honor reduced
  motion.
