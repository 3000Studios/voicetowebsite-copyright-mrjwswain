...
export function buildPredictiveQueue({ intentBundle, strategy }) {
  ...
  if (intents.has("enable_ads")) {
    q.push("add above-the-fold ad placement", "add in-article ad slots", "verify ads.txt + policy pages");
  }

  if (intents.has("enable_email_capture")) {
    q.push("add lead magnet", "add newsletter popup", "enable newsletter drip campaign");
  }

  if (String(strategy) === "hybrid") {
    q.push("enable analytics", "add conversion dashboard", "run A/B test on hero CTA");
  }
